...
void CameraApp::BuildShadersAndInputLayout()
{
    ...
    defaultInputLayout =
    {
        ...
        { "MATINDEX", 0, DXGI_FORMAT_R16_UINT, 0, 32, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },
    };
}
...
void CameraApp::BuildShapeGeometry()
{
    ...
    boxSubmesh.IndexCount = (UINT)box.Indices32.size() * numBox;
    ...
    {
        for(int j = 0; j < box.Vertices.size(); ++j)
        {
            Vertex v;
            v.Pos = box.Vertices[j].Position;
            v.Pos.x += 2.0f * i;

            v.Normal = box.Vertices[j].Normal;
            v.TexC = box.Vertices[j].TexC;
            v.MatIndex = i;
            vertices.push_back(v);
        }

        auto offset = i * box.Vertices.size();
        for(int j = 0; j < boxIndices.size(); ++j)
        {
            indices.push_back(boxIndices[j] + offset);
        }
    }

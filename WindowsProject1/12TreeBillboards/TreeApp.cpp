...
void TreeApp::AnimateMaterials(const GameTimer& gt)
{
...
void TreeApp::Update(const GameTimer& gt)
{
	OnKeyboardInput(gt);
	UpdateCamera(gt);
...
void TreeApp::Draw(const GameTimer& gt)
{
...
void TreeApp::OnMouseLeftDown(int x, int y, short keyState)
{
	lastMousePos.x = x;
	lastMousePos.y = y;
...
void TreeApp::OnMouseLeftUp(int x, int y, short keyState)
{
	ReleaseCapture();
}

void TreeApp::OnMouseMove(int x, int y, short keyState)
{
...
void TreeApp::OnKeyboardInput(const GameTimer& gt)
{
...
void TreeApp::UpdateCamera(const GameTimer& gt)
{
...
void TreeApp::UpdateObjectCBs(const GameTimer& gt)
{
...
void TreeApp::UpdateMaterialCBs(const GameTimer& gt)
{
...
void TreeApp::UpdateMainPassCB(const GameTimer& gt)
{
...
void TreeApp::UpdateWaves(const GameTimer& gt)
{
...
void TreeApp::LoadTexture(std::wstring filePath, std::string textureName)
{
...
void TreeApp::LoadTextureArray(std::wstring filePath, std::string textureName)
{
	auto texture = std::make_unique<Texture>();
	texture->Name = textureName;
	texture->Filename = filePath;
...
std::array<const CD3DX12_STATIC_SAMPLER_DESC, 6> TreeApp::GetStaticSamplers()
{
...
void TreeApp::BuildRootSignature()
{
...
void TreeApp::BuildDescriptorHeaps()
{
	D3D12_DESCRIPTOR_HEAP_DESC srvHeapDesc = {};
	srvHeapDesc.NumDescriptors = textures.size() + textureArrays.size();
...
void TreeApp::BuildShaderResourceViews()
{
	int textureIndex = 0;
...
void TreeApp::BuildShadersAndInputLayout()
{
	...
	shaders["standardVS"] = DxUtil::CompileShader(L"12TreeBillboards\\Shaders\\Default.hlsl", nullptr, "VS", "vs_5_0");
	shaders["opaquePS"] = DxUtil::CompileShader(L"12TreeBillboards\\Shaders\\Default.hlsl", defines, "PS", "ps_5_0");
...
void TreeApp::BuildLandGeometry()
{
...
void TreeApp::BuildWavesGeometryBuffers()
{
...
void TreeApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
...
float TreeApp::GetHillsHeight(float x, float z)const
{
	return 0.3f * (z * sinf(0.1f * x) + x * cosf(0.1f * z));
}

XMFLOAT3 TreeApp::GetHillsNormal(float x, float z)const
{
	// n = (-df/dx, 1, -df/dz)
	XMFLOAT3 n(

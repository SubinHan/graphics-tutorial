...
bool ShadowMapApp::Initialize()
{
    ...
    mShadowMap = std::make_unique<ShadowMap>(
        device->GetD3DDevice().Get(), 256, 256);
...
void ShadowMapApp::OnMouseLeftDown(int x, int y, short keyState)
{
    lastMousePos.x = x;
    lastMousePos.y = y;
...
void ShadowMapApp::OnMouseLeftUp(int x, int y, short keyState)
{
    ReleaseCapture();
}

void ShadowMapApp::OnMouseMove(int x, int y, short keyState)
{
...
void ShadowMapApp::OnKeyDown(WPARAM windowVirtualKeyCode)
{
...
void ShadowMapApp::UpdateCamera(const GameTimer& gt)
{
...
void ShadowMapApp::UpdateInstanceBuffer(const GameTimer& gt)
{
    int bufferOffset = 0;
...
void ShadowMapApp::LoadTexture(std::wstring filePath, std::string textureName)
{
...
void ShadowMapApp::LoadCubeMap(std::wstring filePath, std::string textureName)
{
...
void ShadowMapApp::BuildDescriptorHeaps()
{
    ...
    srvHeapDesc.NumDescriptors = textures.size() + cubeMaps.size() + 3;
    ...
    mShadowMapHeapIndex = descriptorIndex++;
    mNullCubeSrvIndex = descriptorIndex++;
    mNullTexSrvIndex = descriptorIndex++;

    CD3DX12_CPU_DESCRIPTOR_HANDLE hDescriptor(srvDescriptorHeap->GetCPUDescriptorHandleForHeapStart());
    hDescriptor.Offset(mNullCubeSrvIndex, device->GetCbvSrvUavDescriptorSize());

    auto& cubeMap = cubeMaps["skyCubeMap"];
	auto cubeMapResource = cubeMap->Resource;
    ...
    device->GetD3DDevice()->CreateShaderResourceView(nullptr, &srvDesc, hDescriptor);
    ...
    auto dsvCpuStart = device->dsvHeap->GetCPUDescriptorHandleForHeapStart();

    mNullSrv = CD3DX12_GPU_DESCRIPTOR_HANDLE(srvGpuStart, mNullCubeSrvIndex, device->GetCbvSrvUavDescriptorSize());
    auto nullSrv = CD3DX12_CPU_DESCRIPTOR_HANDLE(srvCpuStart, mNullTexSrvIndex, device->GetCbvSrvUavDescriptorSize());
    ...
    mShadowMap->BuildDescriptors(
        CD3DX12_CPU_DESCRIPTOR_HANDLE(srvCpuStart, mShadowMapHeapIndex, device->GetCbvSrvUavDescriptorSize()),
        CD3DX12_GPU_DESCRIPTOR_HANDLE(srvGpuStart, mShadowMapHeapIndex, device->GetCbvSrvUavDescriptorSize()),
        CD3DX12_CPU_DESCRIPTOR_HANDLE(dsvCpuStart, 1, device->GetDsvDescriptorSize()));
...
void ShadowMapApp::BuildRootSignature()
{
    ...
    cubeMapTable.Init(
        D3D12_DESCRIPTOR_RANGE_TYPE_SRV, cubeMaps.size() + 1, 0, 0
    );

    CD3DX12_DESCRIPTOR_RANGE texTable;
    texTable.Init(
        D3D12_DESCRIPTOR_RANGE_TYPE_SRV, textures.size(), 2, 0
    );
    ...
    auto staticSamplers = 
        DxUtil::GetStaticSamplersWithShadowSampler();
...
void ShadowMapApp::BuildShadersAndInputLayout()
{
    ...
    shaders["standardVS"] = DxUtil::CompileShader(L"20ShadowMapping\\Shaders\\Default.hlsl", nullptr, "VS", "vs_5_1");
    shaders["opaquePS"] = DxUtil::CompileShader(L"20ShadowMapping\\Shaders\\Default.hlsl", nullptr, "PS", "ps_5_1");
    shaders["skyVS"] = DxUtil::CompileShader(L"20ShadowMapping\\Shaders\\sky.hlsl", nullptr, "VS", "vs_5_1");
    shaders["skyPS"] = DxUtil::CompileShader(L"20ShadowMapping\\Shaders\\sky.hlsl", nullptr, "PS", "ps_5_1");

	shaders["shadowVS"] = DxUtil::CompileShader(L"20ShadowMapping\\Shaders\\shadow.hlsl", nullptr, "VS", "vs_5_1");
	shaders["shadowOpaquePS"] = DxUtil::CompileShader(L"20ShadowMapping\\Shaders\\shadow.hlsl", nullptr, "PS", "ps_5_1");
	shaders["shadowOpaquePS"] = DxUtil::CompileShader(L"20ShadowMapping\\Shaders\\shadow.hlsl", alphaTestDefines, "PS", "ps_5_1");

    shaders["debugVS"] = DxUtil::CompileShader(L"20ShadowMapping\\Shaders\\ShadowDebug.hlsl", nullptr, "VS", "vs_5_1");
    shaders["debugPS"] = DxUtil::CompileShader(L"20ShadowMapping\\Shaders\\ShadowDebug.hlsl", nullptr, "PS", "ps_5_1");
...
void ShadowMapApp::BuildPSOs()
{
    ...
    ThrowIfFailed(
        device->GetD3DDevice()->CreateGraphicsPipelineState(
            &smapPsoDesc, IID_PPV_ARGS(&pipelineStateObjects["shadow_opaque"])));
    ...
    ThrowIfFailed(
        device->GetD3DDevice()->CreateGraphicsPipelineState(
			&debugPsoDesc, IID_PPV_ARGS(&pipelineStateObjects["debug"])));
...
void ShadowMapApp::BuildFrameResources()
{
    for (int i = 0; i < gNumFrameResources; ++i)
    {
        frameResources.push_back(std::make_unique<FrameResource>(device->GetD3DDevice().Get(),
            2, instanceCount, static_cast<UINT>(materials.size())));
    }
}

void ShadowMapApp::BuildSkullRenderItem(std::unique_ptr<InstancedRenderItem>& skullRitem)
{
...
void ShadowMapApp::DrawSceneToShadowMap()
{
    ...
    auto makeShadowMapWritable =
        CD3DX12_RESOURCE_BARRIER::Transition(mShadowMap->Resource(),
            D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_STATE_DEPTH_WRITE
        );

    // Change to DEPTH_WRITE.
    commandList->ResourceBarrier(1, &makeShadowMapWritable);
    ...
    commandList->SetPipelineState(pipelineStateObjects["shadow_opaque"].Get());

    DrawRenderItemsWithoutFrustumCulling(commandList.Get(), RitemLayer[static_cast<INT>(RenderLayer::OpaqueNonFrustumCull)]);

    // Change back to GENERIC_READ so we can read the texture in a shader.
    auto makeShadowMapReadable = 
        CD3DX12_RESOURCE_BARRIER::Transition(
			mShadowMap->Resource(),
			D3D12_RESOURCE_STATE_DEPTH_WRITE, 
            D3D12_RESOURCE_STATE_GENERIC_READ
        );
    commandList->ResourceBarrier(1, &makeShadowMapReadable);
}


void ShadowMapApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<InstancedRenderItem*>& ritems)
{
    for(auto e : ritems)
    {
...
void ShadowMapApp::DrawRenderItemsWithoutFrustumCulling(ID3D12GraphicsCommandList* cmdList, const std::vector<InstancedRenderItem*>& ritems)
{
    for (auto e : ritems)
    {

...
void NormalMapApp::OnMouseLeftDown(int x, int y, short keyState)
{
    lastMousePos.x = x;
    lastMousePos.y = y;
...
void NormalMapApp::OnMouseLeftUp(int x, int y, short keyState)
{
    ReleaseCapture();
}

void NormalMapApp::OnMouseMove(int x, int y, short keyState)
{
...
void NormalMapApp::OnKeyDown(WPARAM windowVirtualKeyCode)
{
...
void NormalMapApp::UpdateInstanceBuffer(const GameTimer& gt)
{
    int bufferOffset = 0;
...
void NormalMapApp::LoadTexture(std::wstring filePath, std::string textureName)
{
...
void NormalMapApp::LoadCubeMap(std::wstring filePath, std::string textureName)
{
...
{
    shaders["standardVS"] = DxUtil::CompileShader(L"19NormalMapping\\Shaders\\Default.hlsl", nullptr, "VS", "vs_5_1");
    shaders["opaquePS"] = DxUtil::CompileShader(L"19NormalMapping\\Shaders\\Default.hlsl", nullptr, "PS", "ps_5_1");
    shaders["skyVS"] = DxUtil::CompileShader(L"19NormalMapping\\Shaders\\sky.hlsl", nullptr, "VS", "vs_5_1");
    shaders["skyPS"] = DxUtil::CompileShader(L"19NormalMapping\\Shaders\\sky.hlsl", nullptr, "PS", "ps_5_1");
...
void NormalMapApp::BuildSkullRenderItem(std::unique_ptr<InstancedRenderItem>& skullRitem)
{
...
void NormalMapApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<InstancedRenderItem*>& ritems)
{
    for(auto e : ritems)
    {
...
void NormalMapApp::DrawRenderItemsWithoutFrustumCulling(ID3D12GraphicsCommandList* cmdList, const std::vector<InstancedRenderItem*>& ritems)
{
    for (auto e : ritems)
    {

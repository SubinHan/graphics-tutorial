...
void BlendApp::OnMouseLeftDown(int x, int y, short keyState)
{
	lastMousePos.x = x;
	lastMousePos.y = y;
...
void BlendApp::OnMouseLeftUp(int x, int y, short keyState)
{
	ReleaseCapture();
}

void BlendApp::OnMouseMove(int x, int y, short keyState)
{
...
void BlendApp::LoadTexture(std::wstring filePath, std::string textureName)
{
...
void BlendApp::BuildShaderResourceViews()
{
	int textureIndex = 0;
...
void BlendApp::BuildShadersAndInputLayout()
{
	...
	shaders["standardVS"] = DxUtil::CompileShader(L"10Blend\\Shaders\\Default.hlsl", nullptr, "VS", "vs_5_0");
	shaders["opaquePS"] = DxUtil::CompileShader(L"10Blend\\Shaders\\Default.hlsl", defines, "PS", "ps_5_0");

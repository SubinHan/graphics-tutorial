...
void PickingApp::Draw(const GameTimer& gt)
{
    ...
    DrawRenderItemsWithInstancing(
        commandList.Get(), RitemLayer[static_cast<int>(RenderLayer::Highlight)]);
...
void PickingApp::OnMouseLeftDown(int x, int y, short keyState)
{
    lastMousePos.x = x;
    lastMousePos.y = y;

    Pick(lastMousePos.x, lastMousePos.y);
    SetCapture(m_hwnd);
}

void PickingApp::OnMouseLeftUp(int x, int y, short keyState)
{
    ReleaseCapture();
}

void PickingApp::OnMouseMove(int x, int y, short keyState)
{
...
void PickingApp::OnKeyDown(WPARAM windowVirtualKeyCode)
{
...
void PickingApp::Pick(int sx, int sy)
{
    ...
    float vx = (+2.0f * sx / device->GetClientWidth() - 1.0f) / p(0, 0);
    float vy = (-2.0f * sy / device->GetClientHeight() + 1.0f) / p(1, 1);
    ...
    for(auto ri : RitemLayer[static_cast<int>(RenderLayer::OpaqueFrustumCull)])
    {
        ...
        for(auto instance : ri->Instances)
        {
            ...
            if (!ri->Bounds.Intersects(rayOrigin, rayDir, tminLocal))
                continue;
            ...
            for (UINT i = 0; i < triCount; ++i)
            {
                ...
                pickedRitem->Visible = true;
                pickedRitem->IndexCount = 3;
                pickedRitem->BaseVertexLocation = 0;
                pickedRitem->StartIndexLocation = 3 * pickedTriangle;
                
                pickedRitem->Instances[0].World = instance.World;

                pickedRitem->Instances[0].TexTransform = instance.TexTransform;
                pickedRitem->Instances[0].MaterialIndex = instance.MaterialIndex;
...
void PickingApp::UpdateCamera(const GameTimer& gt)
{
...
void PickingApp::UpdateInstanceBuffer(const GameTimer& gt)
{
    XMMATRIX view = camera.GetView();
...
void PickingApp::LoadTexture(std::wstring filePath, std::string textureName)
{
...
void PickingApp::BuildPSOs()
{
    ...
    auto highlightPsoDesc = transparentPsoDesc;
    ...
    ThrowIfFailed(
        device->GetD3DDevice()->CreateGraphicsPipelineState(
            &highlightPsoDesc, IID_PPV_ARGS(&pipelineStateObjects["highlight"])
        )
    );
}
...
void PickingApp::BuildRenderItems()
{
    ...
    picked->Geo = geometries["skullGeo"].get();
    picked->PrimitiveType = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
    ...
    picked->IndexCount = 0;
    picked->StartIndexLocation = 0;
    picked->BaseVertexLocation = 0;
    picked->Bounds = geometries["skullGeo"]->DrawArgs["skull"].Bounds;
    pickedRitem = picked.get();

    pickedRitem->InstanceCount = 1;
    pickedRitem->Instances.resize(1);

    RitemLayer[static_cast<int>(RenderLayer::Highlight)].push_back(picked.get());


    allRitems.push_back(std::move(skullRitem));
    allRitems.push_back(std::move(picked));
}

void PickingApp::DrawRenderItemsWithInstancing(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{

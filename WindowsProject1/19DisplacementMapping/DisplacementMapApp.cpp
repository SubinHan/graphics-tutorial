...
DisplacementMapApp::DisplacementMapApp(HINSTANCE hInstance)
    : MainWindow(hInstance)
{
}

DisplacementMapApp::~DisplacementMapApp()
{
    if (device->GetD3DDevice() != nullptr)
        device->FlushCommandQueue();
}

bool DisplacementMapApp::Initialize()
{
...
void DisplacementMapApp::OnResize()
{
    MainWindow::OnResize();
...
void DisplacementMapApp::Draw(const GameTimer& gt)
{
    auto commandList = device->GetCommandList();
    auto commandQueue = device->GetCommandQueue();
    ...
    if (isWireframe)
        commandList->SetPipelineState(pipelineStateObjects["opaque_wireframe"].Get());
    else
...
void DisplacementMapApp::OnMouseLeftDown(int x, int y, short keyState)
{
    lastMousePos.x = x;
    lastMousePos.y = y;
...
void DisplacementMapApp::OnMouseLeftUp(int x, int y, short keyState)
{
    ReleaseCapture();
}

void DisplacementMapApp::OnMouseMove(int x, int y, short keyState)
{
...
void DisplacementMapApp::OnKeyDown(WPARAM windowVirtualKeyCode)
{
...
void DisplacementMapApp::UpdateCamera(const GameTimer& gt)
{
...
void DisplacementMapApp::UpdateMainPassCB(const GameTimer& gt)
{
...
void DisplacementMapApp::UpdateMaterialBuffer(const GameTimer& gt)
{
...
void DisplacementMapApp::LoadTexture(std::wstring filePath, std::string textureName)
{
...
void DisplacementMapApp::LoadCubeMap(std::wstring filePath, std::string textureName)
{
...
void DisplacementMapApp::LoadTextures()
{
    ...
    LoadTexture(L"Exercise Media/waves0.dds", "waves0Normal");
    LoadTexture(L"Exercise Media/waves1.dds", "waves1Normal");
    LoadTexture(L"Exercise Media/waves0_diff.dds", "waves0Displacement");
    LoadTexture(L"Exercise Media/waves1_diff.dds", "waves1Displacement");
    LoadTexture(L"Textures/black1x1_diff.dds", "emptyDisplacement");
}

void DisplacementMapApp::BuildDescriptorHeaps()
{
    //
    // Create the SRV heap.
    //
...
void DisplacementMapApp::BuildRootSignature()
{
...
void DisplacementMapApp::BuildShadersAndInputLayout()
{
    shaders["standardVS"] = DxUtil::CompileShader(L"19DisplacementMapping\\Shaders\\Default.hlsl", nullptr, "VS", "vs_5_1");
    shaders["opaquePS"] = DxUtil::CompileShader(L"19DisplacementMapping\\Shaders\\Default.hlsl", nullptr, "PS", "ps_5_1");
    shaders["skyVS"] = DxUtil::CompileShader(L"19DisplacementMapping\\Shaders\\sky.hlsl", nullptr, "VS", "vs_5_1");
    shaders["skyPS"] = DxUtil::CompileShader(L"19DisplacementMapping\\Shaders\\sky.hlsl", nullptr, "PS", "ps_5_1");
    shaders["tessVS"] = DxUtil::CompileShader(L"19DisplacementMapping\\Shaders\\Tessellation.hlsl", nullptr, "VS", "vs_5_1");
...
void DisplacementMapApp::BuildMaterials()
{
    ...
    materials["waterMat"] = std::move(waterMat);
}

void DisplacementMapApp::BuildSkullGeometry()
{
...
void DisplacementMapApp::BuildShapeGeometry()
{
...
void DisplacementMapApp::BuildPSOs()
{
    ...
    ZeroMemory(&skyPsoDesc.HS, sizeof(skyPsoDesc.HS));
    ZeroMemory(&skyPsoDesc.DS, sizeof(skyPsoDesc.DS));
    skyPsoDesc.PS =
    {
...
void DisplacementMapApp::BuildFrameResources()
{
...
void DisplacementMapApp::BuildSkullRenderItem(std::unique_ptr<InstancedRenderItem>& skullRitem)
{
...
void DisplacementMapApp::BuildRenderItems()
{
...
void DisplacementMapApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<InstancedRenderItem*>& ritems)
{
    for(auto e : ritems)
    {
...
void DisplacementMapApp::DrawRenderItemsWithoutFrustumCulling(ID3D12GraphicsCommandList* cmdList, const std::vector<InstancedRenderItem*>& ritems)
{
    for (auto e : ritems)
    {
